...
- Formatting: run `nix fmt` on the `.nix` files you changed before committing (nixfmt via the flake `formatter`), e.g. `nix fmt $(git ls-files -m -o --exclude-standard -- '*.nix')`. Pass explicit paths; a bare `nix fmt` and `nix fmt .` both print a deprecation warning (stdin mode / directory args).
