...
- Prefer established, well-maintained libraries when they reduce overall complexity or improve reliability. Do not reimplement common functionality to avoid adding a dependency.
- Check existing dependencies first, but add a new well-maintained dependency when it clearly reduces code you must own and maintain. Do not assume a library lacks a capability without checking its documentation and types. For non-trivial logic (>~20 lines or edge-case heavy), briefly note what you checked and why it was insufficient if you choose to reimplement.
...
- When researching documentation, API references, or usage examples for a library or framework, if the current harness provides a doc lookup tool (e.g., context7 MCP in OpenCode), use it first; otherwise use web search / source code.
